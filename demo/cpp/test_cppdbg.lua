...
local config = {
  ...
  cwd = demo_dir,
  stopAtEntry = false,
  MIMode = vim.fn.has("mac") == 1 and "lldb" or "gdb",
}

-- gdb's MI variable objects bypass the libstdc++ pretty printers until the
-- session enables them.
if config.MIMode == "gdb" then
  config.setupCommands = { { text = "-enable-pretty-printing", ignoreFailures = true } }
end
